...
module.exports = (app) => {
  ...
  router.param('id', (req, res, next) => app.services.accounts
    .find({ id: req.params.id })
    .then((accs) => {
      if (accs[0].user_id !== req.user.id) throw new ForbiddenError();
      next();
    })
    .catch((err) => next(err)));
  ...
  router.delete('/:id', (req, res, next) => app.services.accounts
